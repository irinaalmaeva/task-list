import tkinter as tk
def add_task():
    task = task_entry.get()
    if task:
        task_listbox.insert(tk.END, task)
        task_entry.delete(0, tk.END)

def delete_task():
    selected_task = task_listbox.curselection()
    if selected_task:
        task_listbox.delete(selected_task)

def mark_task():
    selected_task = task_listbox.curselection()
    if mark_task:
        task_listbox.itemconfig(selected_task, bg="slate blue")

def clear_tasks():
    task_listbox.delete(0, tk.END)


root = tk.Tk()
root.title("Task list")
root.config(background= "DarkSeaGreen1" )

text1 = tk.Label(root, text="Введите вашу задачу", bg="DarkSeaGreen1")
text1.pack(pady=5)

task_entry = tk.Entry(root, width= 30, bg= "LightPink3", fg="black")
task_entry.pack(pady=10,padx=10)

add_task_button = tk.Button(root, text="Добавить задачу", command=add_task)
add_task_button.pack(pady=5)

delete_button = tk.Button(root, text="Удалить задачу", command=delete_task)
delete_button.pack(pady=5)

mark_button = tk.Button(root, text= "Отметить выполненную задачу", command=mark_task)
mark_button.pack(pady=5)

text2 = tk.Label(root, text="Список задач",bg="hot pink")
text2.pack(pady=5)

task_listbox = tk.Listbox(root, height=10, width=50, bg="LightPink1")
task_listbox.pack(pady=10)

clear_button = tk.Button(root, text="Очистить список задач", command=clear_tasks)
clear_button.pack(pady=5)


root.mainloop()

